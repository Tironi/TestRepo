...
const server = new MCPServer({
  name: "manufact-example",
  title: "manufact-example", // display name
  version: "1.0.0",
...
server.tool(
  {
    name: "calculate-sum",
    description: "Calculate the sum of two numbers to demonstrate a custom tool",
    schema: z.object({
  ...
  async ({ a, b }) => {
    return text(`The sum of ${a} and ${b} is ${a + b}`);
  }
);
